...
void subMatInv(double* A, int N, int start) {
  int info = 0;
  int lworkspace = N;
  int *ipiv = new int[N];
  dgetrf_(&N, &N, &A[start], &N, ipiv, &info);
  double* workspace = new double[lworkspace*sizeof(double)];
  dgetri_(&N, &A[start], &N, ipiv, workspace, &lworkspace, &info);
  delete [] ipiv;
  delete [] workspace;
}

void subMatsubVecprod(double* A, double* v, double* u, int Nrow, int Ncol, int startA, int startv, int startu){
  double alpha= 1.0, beta= 0.0;
  char tr= 'T';
  int m= Nrow, n= 1, k=Ncol, lda= Ncol, incx= 1, incy= Nrow;

  dgemm_(&tr,&tr,&m,&n,&k,&alpha,&A[startA],&lda,&v[startv],&incx,&beta,&u[startu],&incy);
}
